...
export const CardOverview: FC<Props> = ({ lottery, onClose }) => {
    ...
    const buyHandler = ()=>{
        if(store.user.balance >= lottery.cost) {
            store.setUser({...store.user, balance: store.user.balance-lottery.cost, hourlyProfit: store.user.hourlyProfit+lottery.hourlyProfit})
            setShowGame(true)
        } else {
            setNotMoney(true)
        }
            ...
            >
                ...
                >
                    ...
                    {showGame ? (
                        <LotteryGame
                            lottery={lottery}
                            maxWin={lottery.rewardRange[1]}
                            onWin={(amount) => {
                                console.log("Виграш:", amount);
                            }}
                        />
                    ) : (
                        <>
                            ...
                            <p className="text-sm text-center italic text-pink-200 mb-4">{lottery.description}</p>
                            ...
                            <p className="text-sm mb-2">
                                🏆 Можна
                                виграти: <strong>{lottery.rewardRange[0]}–{lottery.rewardRange[1]} монет</strong>
                            </p>
